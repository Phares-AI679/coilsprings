...
$sql = "SELECT tblbooking.BookingId as bookid,tblbooking.ProductId as pkgid,tblspareparts.ProductName as Productname,tblbooking.FromDate as fromdate,tblbooking.ToDate as todate,tblbooking.Comment as comment,tblbooking.status as status,tblbooking.RegDate as regdate,tblbooking.CancelledBy as cancelby,tblbooking.UpdationDate as upddate from tblbooking join tblspareparts on tblspareparts.ProductId=tblbooking.ProductId where UserEmail=:uemail";
$query = $dbh->prepare($sql);
...
<?php if($result->status==2)
{
	?><td>Cancelled</td>
<?php } else {?>
<td><a href="history.php?bkid=<?php echo htmlentities($result->bookid);?>" onclick="return confirm('Do you really want to cancel Order')" >Cancel</a></td>
<?php }?>
</tr>
<?php $cnt=$cnt+1; }} ?>
	</table>
